...
public class AStar_Controller extends Controller<MOVE>{
	...
	public MOVE getMove(Game gameState,long timeDue)
	{
		...
		for(MOVE m: availableMoves)
		{
			...
			pathsScores = computePathsAStar(0, 15, m, copyOfgameState);
			
			for(int i: pathsScores) {
				if(i < highestScore) {
					highestScore = i;
					highestMove = m;
				}
			}
			System.out.println("Score after move : " + m + " is : " + pathsScores);
